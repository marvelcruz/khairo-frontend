  ...
  return (
    <>
      ...
      <section className="relative bg-ink-black pt-24 pb-12 sm:pt-32 sm:pb-16 overflow-hidden">
        ...
        <div
          className="absolute top-1/2 right-1/4 w-[500px] h-[400px] rounded-full bg-emerald-600/8 blur-[100px]"
          aria-hidden
        />
        <div className="relative max-w-[1200px] mx-auto px-4 sm:px-6">
          <div className="flex flex-wrap items-center gap-3 mb-5">
            <FlaskConical size={20} className="text-emerald-600" aria-hidden />
            <p className="font-ui text-[13px] font-semibold uppercase tracking-[0.12em] text-emerald-600">
              Science & The Lab
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-5 gap-6 mb-12">
            ...
            <Link href={`/science/${featured1.slug}`} className="lg:col-span-3 group block">
              <div className="relative rounded-[1.5rem] overflow-hidden aspect-video mb-5">
                ...
                <span className="absolute top-4 left-4 bg-emerald-600 text-pure-white text-[11px] font-ui font-bold uppercase tracking-wider px-3 py-1.5 rounded-full">
                  {featured1.category}
                </span>
              </div>
              <h2 className="font-display text-2xl text-ink-black leading-tight mb-2 group-hover:text-emerald-600 transition-colors">
                {featured1.title}
              </h2>
            ...
            <Link href={`/science/${featured2.slug}`} className="lg:col-span-2 group block">
              ...
              <h2 className="font-display text-xl text-ink-black leading-tight mb-2 group-hover:text-emerald-600 transition-colors">
                {featured2.title}
              </h2>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {rest.map((article) => (
              <Link key={article.slug} href={`/science/${article.slug}`} className="group block">
                ...
                <h3 className="font-display text-base text-ink-black leading-snug mb-2 group-hover:text-emerald-600 transition-colors line-clamp-2">
                  {article.title}
                </h3>
      ...
      <section className="bg-charcoal py-14">
        <div className="max-w-[760px] mx-auto px-4 sm:px-6 text-center">
          <BookOpen size={28} className="text-emerald-600 mx-auto mb-4" aria-hidden />
